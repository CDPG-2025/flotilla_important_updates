...
class ClientManager:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            print("[FLOW] client_manager.py: Starting ClientManager run loop")
            stop_event = Event()

            self.logger.info("fedclient.init", "")

            print("[FLOW] client_manager.py: Initializing gRPC server")
            grpc_sync_server = self.grpc_init(stop_event)

            print("[FLOW] client_manager.py: Initializing MQTT client")
            mqtt_server = self.mqtt_init(stop_event)

            print("[FLOW] client_manager.py: Waiting for stop event")
            stop_event.wait()

        except KeyboardInterrupt:
            print("[FLOW] client_manager.py: KeyboardInterrupt received, shutting down")
            self.logger.info(
    ...
    def exit_procedure(self, stop_event, grpc_sync_server, mqtt_server):
        print("[FLOW] client_manager.py: Exit procedure started")
        grpc_sync_server.stop(grace=None)
        stop_event.set()
        mqtt_server.join()
        print("[FLOW] client_manager.py: Exit procedure finished")
